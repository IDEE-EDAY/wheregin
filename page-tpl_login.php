...
if( isset( $_POST['login_field'] ) ){
    if( wp_verify_nonce($_POST['login_field'], 'login') ){
        ...
        if( !is_wp_error( $user ) ){
            ...
            $message = '<div class="alert alert-danger">'.__( 'Invalid password', 'couponxl' ).'</div>';
        }
    }
    else{
                        ...
                        <div class="row">
                            <div class="col-sm-8">                                
                                ...
                                <form method="post" action="<?php echo couponxl_get_permalink_by_tpl( 'page-tpl_login' ) ?>">
                                    ...
                                    <input type="hidden" name="redirect" value="<?php echo !empty( $_SERVER['HTTP_REFERER'] ) ? esc_url( $_SERVER['HTTP_REFERER'] ) : '' ?>">
