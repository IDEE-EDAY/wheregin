<?php
		...
		if( !empty( $offer->offer_store ) ){
			...
			if( !empty( $store ) ){
				...
				$store_id = wp_insert_post(array(
					'post_type' => 'store',
					'post_status' => 'publish',
					'post_title' => $offer->store_title,
					'post_content' => $offer->store_description,
				));
	...
	function couponxl_import_image( $image_url ){
		...
		couponxl_display_import_info( __( 'Downloading image: ', 'couponxl' ).basename( $image_url ) );
	...
	*/
	...
	function couponxl_import_offer( $offer ){
		...
		$insert_args = array(
			'post_type' => 'offer',
			'post_title' => $offer->offer_title,
			'post_content' => $offer->offer_description,
			'post_status' => 'publish',
		);
