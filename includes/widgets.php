...
class Couponxl_Latest_Posts_Comments extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		if( in_array( 'latest', $tabs ) ){
			...
			$recent_posts = wp_get_recent_posts( array( 'numberposts' => $count ), OBJECT );
			$panels .= '<div role="tabpanel" class="tab-pane active" id="latest_'.$rand.'"><ul class="list-unstyled">';
			if( !empty( $recent_posts ) ){
				foreach( $recent_posts as $post ){
					$panels .= '<li><a href="'.get_permalink( $post->ID ).'">'.get_the_title( $post->ID ).'</a><p class="green-text">'.get_the_time( get_option( 'date_format' ) ).'</p></li>';
				}
			}
			$panels .= '</ul></div>';
			$active = true;
